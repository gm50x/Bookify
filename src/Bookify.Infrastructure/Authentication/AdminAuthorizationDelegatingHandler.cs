...
public sealed class AdminAuthorizationDelegatingHandler : DelegatingHandler
{
    ...
    private async Task<AuthorizationToken> GetAuthorizationToken(CancellationToken cancellationToken)
    {
        ...
        var authorizationRequestContent =
            new FormUrlEncodedContent(authorizationRequestParameters);
        ...
        return await authorizationResponse.Content.ReadFromJsonAsync<AuthorizationToken>(CancellationToken.None)
            ?? throw new ApplicationException("Authorization token response is null.");
